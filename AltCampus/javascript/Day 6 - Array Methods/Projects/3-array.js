...
function uniqueArray(arr){
  let newArr = [];
  let j = 0;
  // for(let index in arr) newArr[index] = arr[index];
...
console.log("\nA new array void of any repititions:\n", uniqueArray(words));
...
console.log("\nSearching the array for a non-existant word: ", doesWordExist(words2, "randomWord"));
console.log("Searching the array for an existing word: ", doesWordExist(words2, "truth"));
...
console.log("\nNumber of times matter appears: ", howManyTimes(words3, "matter"));
...
let total = data.reduce(((acc, val) => val.country === "China" ? (acc += 0) : (acc += val.pop)), 0);
console.log("\nTotal population except that of China: ", total);
...
function summarized(arr){
  let summarizedObj = {};
  for(let i = 0, n = arr.length; i < n; i++){
    if(summarizedObj[arr[i]] === undefined){
      summarizedObj[arr[i]] = howManyTimes(arr, arr[i]);
    }
  }
  return summarizedObj;
}

let summarizedResult = summarized(fruitBasket);
console.log("\nSummarizing the object: ", summarizedResult);
...
// ANS: Done in two pairs of for loops instead of one for clarity
let greatestProduct = 0;
// to count iterations
let temp_count = 0;
console.log("\nHorizontal");
for(let i = 0, n = matrix.length; i < n; i++){
  for(let j = 0, m = matrix[i].length; j + 3 < m; j++){
    let product = matrix[i][j] * matrix[i][j + 1] * matrix[i][j + 2] * matrix[i][j + 3];
    if(product > greatestProduct) greatestProduct = product;
    temp_count++;
    console.log("CHECK: %d:- %d | %d", temp_count, product, greatestProduct);
  }
}
console.log("Vertical");
for(let j = 0, n = matrix.length; j < n; j++){
  for(let i = 0, m = matrix[i].length; i + 3 < m; i++){
...
console.log("\nThe greatest product of 4 adjacent numbers in the same line: ", greatestProduct);
