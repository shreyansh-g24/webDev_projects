...
// Adding a blockquote tag to the body
let quote = document.createElement("blockquote");
quote.style.border = "1px solid #dedede";
quote.style.padding = "20px";
quote.style.margin = "20px";
quote.style.textAlign = "center";
quote.innerText = "Press space to change the quote";
document.body.appendChild(quote);

// adding quote to the blockquote
function changeQuote(event, quotes){
    if (event.keyCode === 32) {
        // console.log(event);
        // selecting a random index to select quote
        let random = Math.floor(Math.random() * quotes.length);
        quote.innerText = quotes[random].quoteText + " by " + quotes[random].quoteAuthor;
    }
}

function handleEvent(){
    document.addEventListener("keyup", (event) => changeQuote(event, quotes));
}
handleEvent();
